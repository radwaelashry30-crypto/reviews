...
| CNN2D + RAKE clause localization | Aspect-based sentiment (Task 3) | reuses CNN2D above, 0 extra | inherits CNN2D's 92.01%, no separate ABSA benchmark | Live production (zero extra RAM cost) |
| DistilBERT (consistency-trained) + TF-IDF/LogReg ensemble | Fake-review screening (Task 2) | ~67M + — | 91.25% / 91.25% (individual), see below | Live production, TF-IDF-only mode (257MB DistilBERT component not loaded there) |
...
- **ABSA (Task 3) previously hallucinated aspects never mentioned in the text** — fixed via a keyword-presence gate; unmentioned aspects now report "Not mentioned" instead of a guessed sentiment. The original ~738MB external ABSA model was later replaced entirely with CNN2D scoring the RAKE-located clause for each aspect (zero extra memory, zero new dependencies, evaluated against two smaller external alternatives that were both rejected — one for a hidden TensorFlow dependency, one for being unverified). Tradeoff: no "Neutral" class, and clause-level sentiment approximates aspect-level sentiment rather than a purpose-trained model. See module docstring in `backend/app/ml/absa.py`.
- **Fake-review detection (Task 2) was replaced after two prior checkpoints were confirmed unreliable** (label semantics never verified, predictions flipped from 99.9% to 0.1% confidence under a pure synonym substitution, confirmed structural across two independently trained checkpoints). The current model is a DistilBERT (paraphrase-consistency fine-tuned) + TF-IDF/Logistic-Regression ensemble trained on the Ott et al. Deceptive Opinion Spam Corpus (Cornell, genuinely human-verified deceptive-vs-truthful labels). Large-scale validation over all 320 held-out test reviews (not a handful of examples): 0/300 confidently-wrong verdicts under meaning-preserving paraphrasing (95% CI upper bound 1.3%), 6.2% of predictions honestly reported as `UNCERTAIN` rather than forced into a guess. Full investigation, including the rejected Amazon "spam/non-spam" dataset (found to be a 1:1 proxy for star rating, not genuine labels) and four training iterations: `MODEL_COMPARISON_AUDIT.md` §9. Still off by default (`ENABLE_FAKE_REVIEW_MODULE=false`) -- not for reliability reasons anymore, but because the DistilBERT component (~257MB) is a real memory risk on Render's 512MB free tier. Domain-shift caveat: trained on hotel reviews, applied here to Olist e-commerce reviews, not separately measured.
