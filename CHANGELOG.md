...
## 2026-08-22 — ABSA rebuilt on CNN2D; repo renamed
...
- Renamed the GitHub repository to `baseera-marketplace-analytics`; updated all internal references (README, DEPLOYMENT.md) and confirmed the live Render deployment and CI were unaffected by the rename.
...
## 2026-08-19 — Production OOM/CORS fixes
...
- Added opt-in "advanced" batch-upload mode: aspect breakdown, sentiment trend (weekly, when a date column is present), and most-influential-words — all sampled/bounded, never run on a full large upload unconditionally.
...
- Added Task 2 (aspect-based sentiment) to the live inference pipeline; fixed a real CNN2D negation bug found during that work.
