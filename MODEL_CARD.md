...
- **Fake-review detection (Task 2) is confirmed unreliable, not just unverified.** Beyond the label-semantics ambiguity (the model's `LABEL_0`/`LABEL_1` mapping to fake/real is assumed, not verified against its config), predictions were shown to flip from 99.9% to 0.1% confidence under a pure synonym substitution with identical meaning -- confirmed structural, not one bad checkpoint (a from-scratch retrain on a real, verified dataset failed the same way). Off by default (`ENABLE_FAKE_REVIEW_MODULE=false`); when enabled, every prediction is automatically re-scored against a meaning-preserving reword of itself, and a verdict that doesn't hold up is withheld from the UI rather than shown at full weight (`app/ml/fake_review_detection.py`'s `score_with_stability_check`).
